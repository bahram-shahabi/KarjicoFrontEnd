...
export default {
  ...
  created () {
    ...
    axios
      .post('https://karjico.ir/api/user/upgradeUserPlanInfo', formDataV,
        {
          headers: {
  ...
  methods: {
    calcPrice (id) {
      ...
      for (let i = this.formData.userLevel; i < id; i++) {
        count = count + parseInt(this.formData.levelList[i].pay_price)
      }
    ...
    upgradeLevel () {
      ...
      this.$swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          const thisData = this
          const formDataV = new FormData()
          formDataV.append('levelUpgrade', this.selectLevelId)
          formDataV.append('amount', this.price)
          formDataV.append('payType', this.payType)
          axios
            .post('https://karjico.ir/api/user/upgradeUserPlan1', formDataV,
              {
                headers: {
                  'Content-Type': 'multipart/form-data',
                  Authorization: 'Bearer ' + localStorage.getItem('api_token')
            ...
            .then(function (response) {
              ...
              location.replace('https://app.karjico.ir/advPlan/upgradePlan')
